...
const prettyTitle = require('../../lib/prettyTitle');

describe('#prettyTitle', () => {
	it('should make names look nice', () => {
	...
	it('should globally upper case id, ids and url', () => {
		...
		assert.strictEqual(
			prettyTitle('list_ids_url_name'),
			'List IDs URL name',
		);
		assert.strictEqual(
			prettyTitle('list_url_ids_name'),
			'List URL IDs name',
		);
		assert.strictEqual(
			prettyTitle('list_url_ids_name_ddl'),
			'List URL IDs name DDL',
		);
	});
});
