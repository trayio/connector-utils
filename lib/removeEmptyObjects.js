...
const removeEmptyObjects = collection => {
	if (Array.isArray(collection)) {
		const emptied_array = collection.map(removeEmptyObjects);
		return _.compact(emptied_array);
	}
	if (_.isPlainObject(collection)) {
		const emptied_object = _.reduce(
			collection,
			(acc, value, key) => {
				if (
					!(
		...
		return _.isEmpty(emptied_object) ? null : emptied_object;
	} else {
